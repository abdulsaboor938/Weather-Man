...
def handle_e_command(year, dir_path=DATA_DIR):
    ...
    for i in files:
        parsed_data = parse_file((DATA_DIR+'/'+i),['PKT','Max TemperatureC','Min TemperatureC','Max Humidity'])

        # adding to datastructure
        max_temp['value'].append(max(parsed_data['Max TemperatureC']))
        max_temp['date'].append(parsed_data['PKT'][parsed_data['Max TemperatureC'].index(max_temp['value'][-1])])

        min_temp['value'].append(min(parsed_data['Min TemperatureC']))
        min_temp['date'].append(parsed_data['PKT'][parsed_data['Min TemperatureC'].index(min_temp['value'][-1])])
